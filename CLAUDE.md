...
- **`python/src/lib.rs`** — `#[pymodule] fn _native`: pyclasses (`NativeH2Result`, `NativePartitionedH2Result`, `NativeRgResult`, `NativeLdScoreResult`, `NativeH2FileResult`, `NativeOverlapEnrichmentResult`, `NativeMungeSummary`) and pyfunctions. In-memory: `fit_h2`/`fit_h2_partitioned`, `fit_rg`/`fit_rg_partitioned`, `compute_ld_scores_from_bytes`. File-oriented: `estimate_h2` (K==1/K>1, plus `overlap_annot` S-LDSC enrichment and `out`/`print_coefficients` to write `.results`), `estimate_rg`, `estimate_ldscore` (plus `annot`/`thin_annot` for partitioned LD scores, and `out` to write `.l2.ldscore.gz`/`.l2.M`/`.l2.M_5_50` for downstream `estimate_h2`/`estimate_rg`), `munge_sumstats`. Every Rust call runs inside `py.detach(...)` to release the GIL.
...
- **`python/python/ldsc_rs/__init__.py`** — Typed dataclasses (`Estimate`, `HeritabilityResult`, `PartitionedHeritabilityResult`, `GeneticCorrelationResult`, `LdScoreResult`, `H2FileResult`, `OverlapEnrichmentCategory`, `MungeSummary`) wrapping the native pyclasses, plus the public Python-facing functions.
- S-LDSC (partitioned LD scores + `--overlap-annot` enrichment) is supported; see `python/README.md` for usage. Deliberately out of scope: `--h2-cts`, jackknife-diagnostics printing (`--print-cov`/`--print-delete-vals`). See `python/README.md` for the full scope.
